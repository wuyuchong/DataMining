...
Logstic = LogsiticRegression(X, y, threshold = 0.5)    
...
for epoch in range(epochs):
    for x, y_true in zip(X, y):
        ...
        # 输出层
        d_ypred_d_w5 = h1 * sigmoidDerivative(sum_o1)
        d_ypred_d_w6 = h2 * sigmoidDerivative(sum_o1)
        d_ypred_d_b3 = sigmoidDerivative(sum_o1)

        d_ypred_d_h1 = w5 * sigmoidDerivative(sum_o1)
        d_ypred_d_h2 = w6 * sigmoidDerivative(sum_o1)

        # 隐藏层 1
        d_h1_d_w1 = x[0] * sigmoidDerivative(sum_h1)
        d_h1_d_w2 = x[1] * sigmoidDerivative(sum_h1)
        d_h1_d_b1 = sigmoidDerivative(sum_h1)

        # 隐藏层 2
        d_h2_d_w3 = x[0] * sigmoidDerivative(sum_h2)
        d_h2_d_w4 = x[1] * sigmoidDerivative(sum_h2)
        d_h2_d_b2 = sigmoidDerivative(sum_h2)
    ...
    def train(self):
        for epoch in range(self.epochs):
            for x, y_true in zip(self.X, self.y):
                ...
                d_ypred_d_h1 = self.w5 * sigmoidDerivative(sum_o1)
                d_ypred_d_h2 = self.w6 * sigmoidDerivative(sum_o1)
